...
- The TypeScript packages are the product on Node/Electron, browsers/PWA and React Native (iOS, Android). Keep `@byokit/accounts`' browser and React Native export free of runtime Node/Pi imports (`packages/accounts/test/portable.test.ts`); platform usage is in `packages/accounts/README.md`, checks in `CONTRIBUTING.md`.
