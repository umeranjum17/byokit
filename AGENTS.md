...
- The TypeScript packages are the product on every platform: Node/Electron, browsers/PWA, React Native (iOS, Android). `@byokit/accounts` splits by export condition: `portable.ts` (the `react-native` and `browser` side: its own fetch-only ChatGPT device-code engine, since Pi's flows need Node) must never import Node or Pi at run time (`test/portable.test.ts` enforces it); `index.ts` adds Pi's engine, the loopback listener and file stores for computers. Platform proofs: `npm run test:browser` (`examples/pwa`, Playwright), `examples/expo` (`npm run typecheck`, `expo export`, `e2e-android.sh <emulator>`).
